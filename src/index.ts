...
// Initialize environment from candidate locations
import { dirname } from 'path';
import fs from 'fs';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const envCandidates = [
  path.join(__dirname, '../../.env'),          // servers/.env
  path.join(__dirname, '../../../.env'),       // gpt5mcp/.env (one level above servers)
...
for (const candidate of envCandidates) {
  try {
    if (fs.existsSync(candidate)) {
      ...
      console.error("Environment loaded from:", candidate);
      break;
    }
  } catch (e) {
